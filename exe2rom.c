// exe2rom.cpp : Defines the entry point for the console application.
// A better version of EXE2ROM/GLA2ROM/OPT2ROM and alike scheisse ;)
...
struct MZEXE {
	...
	uint16_t PGSZ_L;			//Number of bytes in last 512-byte page
	uint16_t PGSZ;			// Total number of 512-byte pages
	uint16_t RELO;			//Number of relocation entries
	uint16_t HDRSZ;			// Header size in paragraphs
	uint16_t MINPAR;		//Minimum paragraphs of memory
	uint16_t MAXPAR;		//Maximum number of paragraphs
	uint16_t SSINIT;			//Initial SS relative to start
	uint16_t SPINIT;			//Initial SP
	uint16_t CRC16;			//Checksum (or 0) of executable
	uint16_t CSIP;			//CS:IP relative to start of executable
	uint16_t RELOC;			//Offset of relocation table;
	uint16_t OVL;				//Overlay number (0h = main program)
	uint8_t filler[0x1e4];//		DB	1E4H DUP(?)
	};
...
int main(int argc, char* argv[]) {
	...
	fprintf(stderr,"EXE2ROM by G.Dar 2025\r\n");

/* Estrae opzioni della command line: /s */
	// https://stackoverflow.com/questions/9642732/parsing-command-line-arguments-in-c
	opts[0]=0;
	for(i=1; i<argc; i++) {
		if(*argv[i]=='/' || *argv[i]=='-') {
			strcat(opts,argv[i]+1);
			for(j=i;j<argc-1;j++) 
	...
	if(argc < 2) {
		puts("Sintassi: EXE2ROM <file sorgente> [file destinazione] [-Sdimensione] [-noChecksum]");
		return 99;
		}

	if(argc < 3) {
		strcpy(nomeFileO,argv[1]);
		if(p=strchr(nomeFileO,'.'))
			*p=0;
		strcat(nomeFileO,".bin");
		}
	else
		strcpy(nomeFileO,argv[2]);

	if(f1=fopen(argv[1],"rb")) {
		if(f2=fopen(nomeFileO,"wb")) {

			buffer=malloc(1048576L);// esageriamo :D

			fread(&mz,sizeof(mz),1,f1);
			fseek(f1,(mz.HDRSZ<<4)+(newsize * (8-1)),SEEK_SET);		// 	convert to bytes. add offset of last 8K chunk of data
//			fseek(f1,sizeof(mz),SEEK_SET);		// 	; add offset of last 8K chunk of data

			if(mz.signature != 'ZM') {
				fprintf(stderr,"%s: file non valido\n",argv[1]);
				goto fine;
				}
...
//			fprintf(stderr,"%x %x\n",mz.HDRSZ,sizeof(mz));
			...
			fwrite(buffer,newsize,1,f2);

			fprintf(stderr,"%s: %06Xh byte copiati",nomeFileO,newsize);
			if(addCRC) 
				fprintf(stderr," (con checksum: %02X)",c);
			else
				fputs(stderr,".\n");
...
fine:
			...
			fprintf(stderr,"%s: impossibile creare file\n",nomeFileO);
			fclose(f1);
			return 1;
			}
		fclose(f1);
