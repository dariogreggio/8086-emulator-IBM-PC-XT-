...
;TEST_CPU2	= 1		; Define to test CPU type by G.Dar and Cosmodoc
...
ROM_DELAY	= 3		; Seconds to wait after expansion ROM inits (keypress will bypass) [was 2
...
CPU_TYPE_8088  =0  ; Intel 8088
CPU_TYPE_8086  =1  ; Intel 8086
CPU_TYPE_V20   =2  ; NEC V20
CPU_TYPE_V30   =3  ; NEC V30
CPU_TYPE_80188 =4  ; Intel 80188
CPU_TYPE_80186 =5  ; Intel 80186
CPU_TYPE_80286 =6  ; Intel 80286
CPU_TYPE_80386 =7  ; Intel 80386 or above
...
;---------------------------------------------------------------------------------------------------
; Changes by Plasma and Dario
;---------------------------------------------------------------------------------------------------
; Version 3.2 Changes:

;	MC146818 RTC support
;	standard 60h/64h keyboard
;	disabled MDA check
;   removed STI (enable IRQ) at BEGINNING OF IRQ handlers! @#£$% idiots!
;	check for BASIC ROMs was faulty, looking for non-existent AA55h at beginning...
;	CURSOR BLINK!
;	removed V20/8088 CPU test (crap
...
macro	entry	addr
	...
	if pad lt 0
; boh		err	'No room for ENTRY point'
	endif
	if pad gt 0
		db	pad dup(090h)
	endif
...
;	dec	ah				; Continue
; boh pare che secondo il frocio qua ci dovrebbero essere 4x8K EEprom, ma ciascuna con la signature all'inizio... CHE NON E' VERO!
...
@@keyb_delay:
	...
	mov	al, 0aah				; Keyboard reset
	out	64h, al 			;   send the request
	in  al, 60h
	mov	al, 0abh				; Keyboard interface test
	out	64h, al 			;   send the request
	in  al, 60h		; DEV'ESSERE 0!
	cmp al,00h

	mov	al, 0aeh				; Keyboard enable
	out	64h, al 			;   send the request
	in  al, 60h
	mov	al, 60h				; Keyboard set control
	out	64h, al 			;   send the request
	mov al, 45h
...
do_boot:
	...
	mov	cx, 6607h			; Set underline cursor color
@@is_mono:
...
proc	clock_check	near
	...
	mov al, 0
					; Check for clock at base port 70h
	out 70h, al
	nop
	nop
	in	al, 71h				;   read BCD seconds/100
...
;	sti					; Key press hardware interrupt
	...
	push	ax				;   save it
	in	al, 64h 			; Get control port status
	push	ax				;   save it
	or	al, 10000000b			; Set "latch" bit to
;tolto  	out	64h, al 			;   acknowledge data
	pop	ax				; Restore control status
	out	64h, al 			;   to enable keyboard
...
str_banner_2	db	CR, LF, 'Upgrades by G.Dar (Miles & Petrosky)', 0
...
ifdef TEST_CPU2
        ...
        ; Here, at least one of the relevant flag bits turned on, so the CPU must be a 386.
